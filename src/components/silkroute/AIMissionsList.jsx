...
import { Sparkles, Play, Pause, AlertTriangle, CheckCircle, Clock, Target, Loader2, GitBranch, Eye, TrendingUp, Plus, Wrench } from "lucide-react";
                      ...
                      >
                        ...
                        <div className="space-y-1 text-xs text-slate-700">
                          {mission.simulation_metadata?.inputs?.length > 0 && (
                            <div><strong>Inputs:</strong> {mission.simulation_metadata.inputs.join(', ')}</div>
                          )}
                          {mission.simulation_metadata?.triggers?.length > 0 && (
                            <div><strong>Triggers:</strong> {mission.simulation_metadata.triggers.join(', ')}</div>
                          )}
                          {mission.simulation_metadata?.safeguards?.length > 0 && (
                            <div><strong>Safeguards:</strong> {mission.simulation_metadata.safeguards.join(', ')}</div>
                          )}
                          {mission.simulation_metadata?.outputs?.length > 0 && (
                            <div><strong>Outputs:</strong> {mission.simulation_metadata.outputs.join(', ')}</div>
                          )}
